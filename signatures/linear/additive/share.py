        ...
        for e in range(self.params.nb_execs):
            ...
            for i in range(self.params.nb_parties):
                ...
                if (i < self.params.nb_parties - 1):
                    raw_share = vec_rnd(self.params.q, self.struct_module.Share.size(self.params), prng)
                    shares[i] = self.struct_module.Share.deserialize(self.params, raw_share)
                    raw_acc = vec_add(raw_acc, raw_share)
                    ...
                    last_unif = vec_rnd(self.params.q, self.struct_module.UniformVector.size(self.params), prng)
                    raw_acc_unif = acc.unif.serialize()
                    raw_plain_unif = vec_add(raw_acc_unif, last_unif)
                    ...
                    last_wtn = vec_sub(raw_plain_wtn, raw_acc_wtn)
                    ...
                    last_corr = vec_sub(raw_plain_corr, raw_acc_corr)
            ...
            for i in range(self.params.nb_parties):
                ...
                if (i < self.params.nb_parties - 1):
                    ...
                    last_unif = self.struct_module.UniformVector.deserialize(
                        self.params, vec_rnd(self.params.q, self.struct_module.UniformVector.size(self.params), prng)
                    )
