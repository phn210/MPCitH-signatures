...
class SharingScheme:
    def __init__(self, params, mseed, salt, struct_module, ff) -> None:
        self.mseed = mseed
        self.salt = salt
        ...
        for e in range(self.params.nb_execs):
            ...
            for i in range(self.params.nb_parties):
                ...
                if (i < self.params.nb_parties - 1):
                    raw_share = self.ff.vec_rnd(self.params.q, self.struct_module.Share.size(self.params), prng)
                    shares[i] = self.struct_module.Share.deserialize(self.params, raw_share)
                    raw_acc = self.ff.vec_add(raw_acc, raw_share)
                    ...
                    last_unif = self.ff.vec_rnd(self.params.q, self.struct_module.UniformVector.size(self.params), prng)
                    raw_acc_unif = acc.unif.serialize()
                    raw_plain_unif = self.ff.vec_add(raw_acc_unif, last_unif)
                    ...
                    last_wtn = self.ff.vec_sub(raw_plain_wtn, raw_acc_wtn)
                    ...
                    last_corr = self.ff.vec_sub(raw_plain_corr, raw_acc_corr)
            ...
            for i in range(self.params.nb_parties):
                ...
                if (i < self.params.nb_parties - 1):
                    ...
                    last_unif = self.struct_module.UniformVector.deserialize(
                        self.params, self.ff.vec_rnd(self.params.q, self.struct_module.UniformVector.size(self.params), prng)
                    )
    ...
    @benchmark
    def recompose_broadcast(self, broadcast: np.ndarray, plain_broadcast: object, hidden_view: int, ff):
        return self.share_ctx.recompose_broadcast(self.params, self.struct_module.BroadcastVector, 
                                                  broadcast, plain_broadcast, hidden_view, ff)