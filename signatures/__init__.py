...
class SignatureScheme:
    ...
    def sign(self, msg: bytes, prvKey: PrivateKey, salt: bytes, seed: bytes) -> bytes:
        ...
        sig_bytes = self.sign_algo.sign(msg, prvKey, salt, seed, self.mpc, self.structs)
        print('Done!')
        return sig_bytes
    ...
    def verify(self, msg: bytes, pubKey: PublicKey, sig: bytes) -> bool:
        ...
        is_correct = self.sign_algo.verify(sig, msg, pubKey, self.mpc, self.structs)
        print('Done!')
        return is_correct
    ...
    def sign_and_verify(self, msg: bytes, prvKey: PrivateKey, pubKey: PublicKey, salt: bytes, seed: bytes):
        ...
        print('Done!')
        print('Verifying...')
        is_correct = self.sign_algo.verify(sig_bytes, msg, pubKey, self.mpc, self.structs)
        print('Done!')
