...
import mpc.minrank.lp.mpc_c as mpc

class MPC:
    def __init__(self, params: Parameters, ff):
        self.params = params
        self.ff = ff

    # @benchmark
    def compute_correlated(self, wtn: Witness, unif: UniformVector) -> CorrelatedVector:
        c = np.zeros((self.params.eta, self.params.m), dtype=np.int32)
        for i in range(self.params.r):
            for j in range(self.params.eta):
                c[j] =  self.ff.ext_add(c[j], self.ff.ext_mul(wtn.beta[i], unif.a[i][j]))
        for j in range(self.params.eta):
            c[j] = self.ff.ext_neg(c[j])
    ...
    #     mat_e = self.ff.matcols_muladd(mat_e, share.wtn.x, inst.mats)
    ...
    #                 br.alpha[i][k] = self.ff.ext_add(br.alpha[i][k], self.ff.ext_mul(chall.gamma[j][k], mat_e[j]))
    #             mat_e[j] = self.ff.ext_powq(mat_e[j])

    #     # Compute alpha
    #     for i in range(self.params.r):
    #         for k in range(self.params.eta):
    #             br.alpha[i][k] = self.ff.ext_add(self.ff.ext_mul(br.alpha[i][k], chall.eps[k]), share.unif.a[i][k])

    #     if entire_computation:
    #         # Compute z
    #         z = np.zeros((self.params.eta, self.params.m), dtype=np.int32)
    #         for j in range(self.params.n):
    #             for k in range(self.params.eta):
    #                 z[k] = self.ff.ext_add(z[k], self.ff.ext_mul(chall.gamma[j][k], mat_e[j]))
    #         for k in range(self.params.eta):
    #             z[k] = self.ff.ext_neg(z[k])
            
    #         # Compute v
    #         for k in range(self.params.eta):
    #             br.v[k] = self.ff.ext_sub(self.ff.ext_mul(z[k], chall.eps[k]), share.corr.c[k])
    #         for i in range(self.params.r):
    #             for k in range(self.params.eta):
    #                 br.v[k] = self.ff.ext_sub(br.v[k], self.ff.ext_mul(plain_br.alpha[i][k], share.wtn.beta[i]))
    #     else:
    #         br.v = np.zeros((self.params.eta, self.params.m), dtype=np.int32)
    ...
    def mpc_compute_plain_broadcast(self, chall: ChallengeVector, share: Share, plain_br: BroadcastVector, inst: Instance) -> BroadcastVector:
        br = BroadcastVector.empty(self.params)
        mpc.run_multiparty_computation(chall.gamma, chall.eps, share.wtn.x, share.wtn.beta, share.unif.a, share.corr.c,
                                        plain_br.alpha, br.alpha, br.v, inst.m0, inst.mats,
                                        1, 0, self.params.r, self.params.n, self.params.m, self.params.eta, self.ff)
        return br
    ...
    def mpc_compute_communications(self, chall: ChallengeVector, share: Share, plain_br: BroadcastVector, inst: Instance,
        ...
        mpc.run_multiparty_computation(chall.gamma, chall.eps, share.wtn.x, share.wtn.beta, share.unif.a, share.corr.c,
                                        plain_br.alpha, br.alpha, br.v, inst.m0, inst.mats,
                                        int(has_sharing_offset), 1, self.params.r, self.params.n, self.params.m, self.params.eta, self.ff)
        return br
    ...
    def mpc_compute_communications_inverse(self, chall: ChallengeVector, share: Share, br: BroadcastVector, plain_br: BroadcastVector, inst: Instance,
        ...
        for i in range(self.params.r):
            for j in range(self.params.n):
                for k in range(self.params.eta):
                    share.unif.a[i][k] = self.ff.ext_add(share.unif.a[i][k], self.ff.ext_mul(chall.gamma[j][k], mat_e[j]))
                mat_e[j] = self.ff.ext_powq(mat_e[j])

        # Compute alpha
        for i in range(self.params.r):
            for k in range(self.params.eta):
                share.unif.a[i][k] = self.ff.ext_sub(br.alpha[i][k], self.ff.ext_mul(share.unif.a[i][k], chall.eps[k]))
        ...
        # Compute v
        for k in range(self.params.eta):
            share.corr.c[k] = self.ff.ext_sub(self.ff.ext_mul(z[k], chall.eps[k]), br.v[k])
        for i in range(self.params.r):
            for k in range(self.params.eta):
                share.corr.c[k] = self.ff.ext_sub(share.corr.c[k], self.ff.ext_mul(plain_br.alpha[i][k], share.wtn.beta[i]))
    ...
    def expand_mpc_challenge_hash(self, digest: bytes, inst: Instance):
        ...
        for i in range(self.params.nb_execs):
            gamma = np.array(self.ff.vec_rnd(self.params.q, ChallengeVector.size_gamma(self.params), prng)) \
                    .reshape((self.params.n, self.params.eta, self.params.m))
            eps = np.array(self.ff.vec_rnd(self.params.q, ChallengeVector.size_eps(self.params), prng)) \
